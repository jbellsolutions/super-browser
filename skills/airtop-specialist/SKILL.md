...
- Cloud browser sessions where Super Browser should open a URL and ask Airtop to query the page.
...
If `AIRTOP_API_BASE` is set, Super Browser validates it before sending `AIRTOP_API_KEY`. Loopback HTTP is allowed for local testing, but private-network/link-local endpoints and insecure remote HTTP require explicit override env vars.

Before opening a target URL in Airtop, Super Browser resolves public-looking hostnames locally. If DNS points to loopback, private-network, or link-local addresses, execution returns `provider_url_resolved_target_scope` and does not create the Airtop session. Treat that as a non-resumable safety stop; create a new run or replan for the intended target scope.
