...
class StoreCliMcpTests(unittest.TestCase):
    ...
    def test_cli_install_skill_rejects_destination_that_contains_source(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertEqual(code, 1)
                self.assertIn("install destination must not contain the Super Browser repository", error_output.getvalue())
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmp:
                ...
                with patch.object(setup_helpers, "__file__", package_file), patch.object(setup_helpers.sys, "prefix", venv), patch.object(
                    ...
                    with self.assertRaisesRegex(ValueError, "install-skill needs a Super Browser source repository"):
    ...
    def test_cli_init_mcp_rejects_invalid_cwd_without_writing_config(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(code, 1)
            self.assertIn("MCP cwd must point to a Super Browser repository or installed bundle", error_output.getvalue())
    ...
    def test_mcp_init_rejects_invalid_cwd_without_writing_config(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "MCP cwd must point to a Super Browser repository or installed bundle"):
    ...
    def test_cli_doctor_reports_actionable_provider_readiness(self):
        ...
        old_browser_use = os.environ.pop("BROWSER_USE_API_KEY", None)
        old_hyperbrowser = os.environ.get("HYPERBROWSER_API_KEY")
        old_state = os.environ.get("SUPER_BROWSER_STATE_DIR")
        try:
            with tempfile.TemporaryDirectory() as tmp:
                os.environ["SUPER_BROWSER_STATE_DIR"] = tmp
            ...
            if old_browser_use is not None:
                os.environ["BROWSER_USE_API_KEY"] = old_browser_use
            if old_hyperbrowser is None:
                os.environ.pop("HYPERBROWSER_API_KEY", None)
            else:
                os.environ["HYPERBROWSER_API_KEY"] = old_hyperbrowser
            if old_state is None:
                os.environ.pop("SUPER_BROWSER_STATE_DIR", None)
            else:
                os.environ["SUPER_BROWSER_STATE_DIR"] = old_state
    ...
    def test_mcp_resources_do_not_follow_symlinks_outside_repo_root(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                with self.assertRaisesRegex(ValueError, "Resource path escapes Super Browser root"):
