...
Super Browser's optional Slack daemon (`super-browser agent`) is a thin ingress: Slack message → plan/run → approval replies in-thread. It reuses the same runtime as CLI and MCP — no second codebase.
...
2. Name it (e.g. `Super Browser`) and pick your workspace.
...
In Slack: `/invite @Super Browser` to a channel, or DM the bot directly.
...
If you use **agent-os** / Hermes, wire the `browser_operator` identity and Super Browser MCP instead of running `super-browser agent`. Slack then goes through Hermes; the browser runtime is the same.
...
| Bot never responds in channel | Invite bot; mention with `@Super Browser` |
