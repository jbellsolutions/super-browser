...
**Status in Super Browser:** `evaluating` (live adapter — Playwright CDP)  
**Docs:** https://docs.steel.dev/  
**Signup:** https://steel.dev/

## Verified in Super Browser

- Creates session via `POST https://api.steel.dev/v1/sessions` unless `STEEL_CDP_URL` override set.
- CDP URL uses API-returned `session.id` (not Super Browser `run_id`).
...
| Capability | Super Browser |
| --- | --- |
| Playwright CDP | **Verified** adapter |
| Puppeteer CDP | Same connect pattern (manual) |
| Selenium | Vendor-supported; not in Super Browser adapter |
