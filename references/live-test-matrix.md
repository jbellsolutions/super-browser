...
Live-test summaries are written to the Super Browser state directory under `live-tests/<provider>.json`. They contain redacted provider, status, timestamp, run id, workflow class, certified workflow classes, per-class latest records, checks, artifact count, event count, and confidence metadata. Set `SUPER_BROWSER_LIVE_EVIDENCE_MAX_AGE_DAYS` to change the default 30-day freshness window. Set `SUPER_BROWSER_RECORD_LIVE_TEST_EVIDENCE=0` to disable evidence writes for one-off CI runs.
