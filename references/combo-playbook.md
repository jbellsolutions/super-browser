...
**Use alone:** `steel` session create + CDP connect (Super Browser adapter).
...
3. Super Browser records artifacts and approval gates for external writes.

**Runtime note:** Super Browser executes step 1 when `steel` is primary. Step 2 is outside the adapter unless you wire a custom agent harness.
