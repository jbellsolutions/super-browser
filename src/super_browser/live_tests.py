...
class _FixtureHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path.endswith(".json"):
            ...
            body = b"<html><head><title>Super Browser Fixture</title></head><body><h1>Fixture Ready</h1><p>Local browser test.</p></body></html>"
            content_type = "text/html"
...
def _run_external_write_gate_fixture(provider_name: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        try:
            os.environ["SUPER_BROWSER_STATE_DIR"] = tmp
            run = create_run("Post this Super Browser live-test comment", url="https://example.com", providers_allowed=[provider_name])
...
def _run_file_upload_fixture(page, base_url: str, artifact_dir: Path) -> dict[str, Any]:
    ...
    upload_path.write_text("Super Browser upload fixture", encoding="utf-8")
