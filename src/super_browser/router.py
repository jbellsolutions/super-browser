...
def _constraint_error(task: TaskSpec) -> str:
    ...
    return "No Super Browser provider satisfies the routing constraints" + (f" ({'; '.join(constraints)})" if constraints else "")
...
def provider_sequence_constraint_failures(plan: Plan | dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not task.url and primary in URL_REQUIRED_PROVIDERS:
        failures.append(
            {
