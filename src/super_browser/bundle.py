...
def _resolve_bundle_root(root: str | Path | None) -> Path:
    if root is None:
        ...
        if discovered is None:
            raise ValueError("Super Browser source root or installed bundle was not found.")
        return discovered
    bundle_root = Path(root).expanduser().resolve()
    if not is_super_browser_root(bundle_root):
        raise ValueError(f"Path is not a Super Browser repository or installed bundle: {bundle_root}")
    return bundle_root
