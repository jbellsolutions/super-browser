...
RESOURCE_FILES = {
    ...
    "super-browser://SKILL": {
        "path": "SKILL.md",
        "name": "Root Skill",
        "description": "Top-level Super Browser skill instructions for agents.",
    },
...
def _run_id_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "properties": {"run_id": {"type": "string", "minLength": 1, "description": "Durable Super Browser run id."}},
        "required": ["run_id"],
...
PRODUCTION_READINESS_INPUT_SCHEMA = {
    ...
    "properties": {
        "required_providers": {
            "type": "array",
            "items": {"type": "string", "enum": PROVIDER_NAMES},
            "description": "Optional production provider subset. Defaults to every Super Browser provider.",
        }
    },
    "additionalProperties": False,
}
...
def _resource_path(item: dict[str, str]) -> Path:
    root = _resource_root()
    if root is None:
        raise ValueError("Super Browser resource docs are unavailable; set SUPER_BROWSER_REPO_ROOT to a Super Browser repository or installed bundle")
    path = (root / item["path"]).resolve()
    try:
        path.relative_to(root)
    except ValueError as exc:
        raise ValueError(f"Resource path escapes Super Browser root: {item['path']}") from exc
    return path
