...
def handle_message(text: str, *, user_id: str = "slack-user", execute: bool = False) -> str:
    """Parse a Slack message and dispatch to the Super Browser runtime."""
...
def run_slack_daemon(*, execute_on_approve: bool | None = None) -> None:
    """Start Slack Socket Mode ingress for Super Browser (optional Level 2 host)."""
    try:
        from slack_bolt import App
        from slack_bolt.adapter.socket_mode import SocketModeHandler
    except ImportError as exc:
        raise SystemExit(
            "slack_bolt is required for the Super Browser Slack daemon. "
    ...
    log.info("Super Browser Slack daemon starting (execute_on_approve=%s)", execute_on_approve)
