...
OPTIONAL_PROVIDER_ENV = {
    "browserbase-stagehand": ["BROWSERBASE_CONTEXT_ID"],
    "rtrvr": ["RTRVR_API_BASE"],
    "orgo": ["ORGO_API_BASE", "ORGO_MODEL"],
    "airtop": ["AIRTOP_API_BASE", "AIRTOP_TIMEOUT_MINUTES"],
    "decodo-http": ["DECODO_PROXY"],
    "hyperbrowser": ["HYPERBROWSER_API_BASE"],
    "steel": ["STEEL_CDP_URL"],
    "browserless": ["BROWSERLESS_BASE_URL"],
}
...
ENV_PURPOSES = {
    ...
    "BROWSERBASE_CONTEXT_ID": "Optional Browserbase persisted context id.",
    "BROWSERBASE_PROJECT_ID": "Browserbase project id for cloud browser sessions.",
    "BROWSERLESS_BASE_URL": "Optional Browserless API base override.",
    ...
    "ORGO_COMPUTER_ID": "Existing Orgo computer id.",
    "ORGO_MODEL": "Optional Orgo model override.",
    "RTRVR_API_BASE": "Optional Rtrvr API base override.",
    "RTRVR_API_KEY": "Rtrvr API credential.",
    ...
    "SUPER_BROWSER_REPO_ROOT": "Optional path to a Super Browser repo or installed bundle for MCP resources.",
...
def _env_item(name: str, *, required: bool, provider: str | None = None, scope: str = "provider") -> dict[str, Any]:
    return {
        ...
        "purpose": ENV_PURPOSES.get(name, "Super Browser configuration value."),
