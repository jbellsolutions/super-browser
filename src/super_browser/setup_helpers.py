...
def repo_root() -> Path:
    ...
    if root:
        return root
    raise ValueError(
        "Super Browser source root was not found. Set SUPER_BROWSER_REPO_ROOT to a Super Browser repository or installed bundle "
    ...
    if root is None:
        if target is None:
            return {
                ...
                "message": (
                    "install-skill needs a Super Browser source repository, installed bundle, or packaged asset tree. "
                    "This package installation did not include the plugin assets."
                ),
            }
        raise ValueError(
            "install-skill needs a Super Browser source repository or installed bundle. Set SUPER_BROWSER_REPO_ROOT to that path, "
...
def _ensure_not_inside_source(source: Path, destination: Path) -> None:
    try:
        ...
        raise ValueError("install target must be outside the Super Browser repository")
    try:
        source.relative_to(destination)
    except ValueError:
        return
    raise ValueError("install destination must not contain the Super Browser repository")
...
def _validated_mcp_root(cwd: str | Path | None = None) -> Path:
    ...
    if not root.is_dir() or not _looks_like_super_browser_root(root) or not server.is_file() or not os.access(server, os.X_OK):
        raise ValueError(
            "MCP cwd must point to a Super Browser repository or installed bundle containing executable mcp/super-browser-server: "
            f"{root}"
        )
    return root
